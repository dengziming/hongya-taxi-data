...
object RunGeoTime extends Serializable {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val taxiRaw = spark.read.option("header", "true").csv("taxispark/src/main/resources/taxidata")
    val taxiParsed = taxiRaw.rdd.map(safe(parse))
    val taxiGood = taxiParsed.filter(_.isLeft).map(_.left.get).toDS
    taxiGood.cache()
    ...
    val geojson = scala.io.Source.fromFile("taxispark/src/main/resources/nyc-boroughs.geojson").mkString
